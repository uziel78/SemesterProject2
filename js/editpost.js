...
import deleteButton from "./components/deleteBtn.js"; "./components/createMenu.js";
...
(async function () {
    try {
        ...
        displayMessage("success", "Edit your Post below, then click Update Post to publish", ".message__container");
        console.log(json);
    } catch (error) {
...
function submitForm(event) {
    ...
    //check for content
    if (titleValue.length === 0 || contentValue.length === 0 || idValue === null) {
...
async function updatePost(title, content, id) {
    ...
    try {
        ...
        if (json.date) {
            displayMessage("success", "Post updated successfully", ".message__container");
        }
